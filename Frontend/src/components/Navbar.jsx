    ...
    <div className="flex items-center justify-between py-5 font-medium">
      ...
      <div className={`absolute top-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
        <div className="flex flex-col text-gray-600">
          ...
          <NavLink onClick={()=> setVisisble(false)} className="py-2 pl-4 mr-4 border mb-2" to="/">HOME</NavLink>
          <NavLink onClick={()=> setVisisble(false)} className="py-2 pl-4 mr-4 border mb-2" to="/collection">COLLECTION</NavLink>
          <NavLink onClick={()=> setVisisble(false)} className="py-2 pl-4 mr-4 border mb-2" to="/about">ABOUT</NavLink>
          <NavLink onClick={()=> setVisisble(false)} className="py-2 pl-4 mr-4 border mb-2" to="/contact">CONTACT</NavLink>
        </div>
      </div>
    </div>
